...
function updateDOM() {
    ...
    events.forEach((event, id) => {
      ...
      title.setAttribute('data-bs-placement', 'top'); // Adjust the placement as needed
      title.setAttribute('title', `Time: ${event.time}, Location: ${event.location}, Attendees: ${event.attendees}`);            // Initialize the tooltip
        new bootstrap.Tooltip(title); //every time the event changes this will change as well, no function needed???
      ...
      document.querySelector(`#${event.day.toLowerCase()} .event-container`).appendChild(eventElement);
    ...
    //I am so confused as to why we need this function when we can just update the tooltip every time updateDOM()
    // is called. 
  ...
  function openEventModal({id, day}) {
    ...
    document.querySelector('#weekday').value = event.day; // Use the provided `day`
